...
class FakeEnv:
    ...
    def step(self, correction_ratios, obs, act, deterministic=False):
        ...
        if not deterministic:
            ...
            # num_models: 7,  batch_size: 50000
            # model_inds: [3 1 6 ... 1 4 3]
            # batch_inds: [0 1 2 ... 49997 49998 49999]
            ...
            # ensemble_samples shape: (7, 50000, 12),  samples shape: (50000, 12)
            ####
        else:
        ...
        #     penalized_rewards = rewards - self.penalty_coeff * penalty  # here to modify
        # else:
        #     penalty = None
        #     unpenalized_rewards = rewards
        ...
        if self.penalty_coeff != 0:
            correction_ratios = np.expand_dims(correction_ratios[np.arange(batch_size), model_inds], 1)
            penalty = self.penalty_function(correction_ratios)
            # penalty = np.log(1e-8 + correction_ratios)
            assert penalty.shape == rewards.shape
            unpenalized_rewards = rewards
            # print('[Reward penalty]', rewards, penalty)
            penalized_rewards = rewards - self.penalty_coeff * penalty  # correction reward
        else:
            penalty = None
            unpenalized_rewards = rewards
            penalized_rewards = rewards
        ...
        print(next_obs.shape, penalized_rewards.shape, terminals.shape)  # check
